...
.panel{position:fixed;top:0;bottom:0;width:330px;background:rgba(12,14,20,.93);backdrop-filter:blur(14px);
...
function ajustar(){
  ...
  const izq = W<900 ? 0 : 345, der = W<760 ? 0 : (W<900 ? 150 : 235), mg=50;
  const dispW=Math.max(W-izq-der-mg*2, 220), dispH=Math.max(H-mg*2, 220);
  cam.z=Math.max(0.12, Math.min(dispW/Math.max(x1-x0,1), dispH/Math.max(y1-y0,1), 1.7));
  cam.x=izq+mg+(dispW-(x1-x0)*cam.z)/2-x0*cam.z;
  cam.y=mg+(dispH-(y1-y0)*cam.z)/2-y0*cam.z;
}
...
window.configurar=configurar; window.guardarCfg=guardarCfg; marcarCfg();
</script></body></html>"""
