...
function alternarMic(){
  ...
  rec = new SR();
  rec.lang = 'es-CL';
  rec.continuous = false;      // un turno cada vez: hablas, contesta, hablas
  rec.interimResults = true;

  const caja = document.getElementById('chat');
  let final = '';

  rec.onstart = () => { escuchando = true; marcarMic(); estado('escuchando'); };
  ...
  rec.onerror = ev => {
    escuchando = false; marcarMic(); estado('');
    const causa = {
  ...
  try { rec.start(); }
  catch(e){ mostrarRta('No pude abrir el micrófono: ' + e.message); }
}

function pararMic(){ try{ rec && rec.stop(); }catch(e){} }
function marcarMic(){
  const b = document.getElementById('btnmic');
  if(b) b.classList.toggle('on', escuchando);
}
